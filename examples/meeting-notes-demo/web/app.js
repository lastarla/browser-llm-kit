...
import buildStructuredRecordPrompt from '../shared/prompts/structured-record-prompt.js';
import buildAchievedResultsPrompt, { buildDefaultAchievedResults } from '../shared/prompts/achieved-results-prompt.js';
...
function ensureWebLlmInstallListeners() {
  ...
  llm.onStatusChange(WEB_LLM_MODEL, (status) => {
    ...
    const installProgress = typeof latestInstallSnapshot?.progress?.percent === 'number'
      ...
      : null;
    const mappedProgress = typeof status === 'string' && status.startsWith('模型安装不可用')
      ? 10
...
function formatInstallHint(snapshot) {
  ...
  if (snapshot.state === 'installed' || snapshot.stored) {
    return '提示：模型资源已进入 OPFS，后续加载会直接复用本地安装。';
  }

  if (snapshot.systemState === 'downloading-partial') {
    return '提示：关闭当前浮层不会中断后台下载；如果关闭标签页或网络中断，重新安装时会尝试续传。';
  }

  if (snapshot.systemState === 'verifying-temp') {
    return '提示：文件已写入 OPFS，正在做完整性校验。';
  }

  if (snapshot.state === 'failed' && ['INSTALL_NETWORK_ERROR', 'INSTALL_CANCELLED'].includes(snapshot.errorCode)) {
    return '提示：安装未完成。重新安装时会优先复用已写入的 OPFS 进度。';
  }
...
function isInstallInFlight(snapshot) {
  return snapshot?.state === 'installing';
}
...
function buildWebLlmDiagnosticsSnapshot(cacheDebug) {
  ...
  const summary = [
    `存储：${cacheDebug.requestPaths.length} 条`,
  ...
  return {
    ...
    detail: JSON.stringify({
      ...
      storage: {
        installId: cacheDebug.cacheName,
        stored: cacheDebug.controller,
        note: cacheDebug.controllerNote,
        installPaths: cacheDebug.requestPaths,
      },
      verificationMethods,
      failedAssets,
      assetSummary,
...
async function collectWebLlmCacheDebug() {
  const diagnostics = llm.getDiagnosticsSnapshot(WEB_LLM_MODEL);
  const install = diagnostics?.install || {};
  const storageConfig = llm.getModelCacheConfig(WEB_LLM_MODEL);
  ...
  const summary = [
    '存储：OPFS',
    `安装 ${install.state || 'absent'}`,
    `模型资源 ${install.stored ? '已落盘' : '未落盘'}`,
  ].join(' | ');

  const detail = JSON.stringify({
    storageBackend: storageConfig.storageBackend,
    installId: storageConfig.installId,
    runtime: storageConfig.runtime,
    installUrls: storageConfig.installUrls,
    runtimeUrls: storageConfig.runtimeUrls,
  ...
  return {
    summary,
    detail,
    cacheName: storageConfig.installId,
    controller: Boolean(install.stored),
    controllerNote: install.stored ? '模型资源已进入 OPFS' : '模型资源尚未进入 OPFS',
    registrations: 0,
    cacheKeys: [],
    requestPaths: storageConfig.installUrls || [],
  };
}
...
async function ensureWebLlmReady() {
  ...
  setWebLlmModelStatus('准备模型安装', 20);
...
async function runWebLlm(index) {
  ...
  setWebLlmCacheDebug('存储：检查中', '正在读取 OPFS 安装状态...');
  renderWebLlmInstallFacts([
  ...
  try {
    ...
    let structuredPreviewRaf = 0;
    const flushStructuredPreview = () => {
      structuredPreviewRaf = 0;
      if (currentRequestId !== webLlmRequestId || structuredTaskId !== webLlmTaskState.structuredTaskId) {
        return;
      }
      setWebLlmCardState('result', '生成中', structuredPreview);
    };
    const scheduleStructuredPreview = () => {
      if (structuredPreviewRaf) {
        return;
      }
      structuredPreviewRaf = window.requestAnimationFrame(flushStructuredPreview);
    };
    const structuredTaskId = llm.submit({
      ...
      options: {
        stream: true,
        callback: (text, _done, delta) => {
          if (currentRequestId !== webLlmRequestId || structuredTaskId !== webLlmTaskState.structuredTaskId) {
            return;
          }
          if (delta) {
            structuredPreview += delta;
            scheduleStructuredPreview();
            return;
          }
          if (text) {
    ...
    if (structuredPreviewRaf) {
      window.cancelAnimationFrame(structuredPreviewRaf);
      structuredPreviewRaf = 0;
    }
